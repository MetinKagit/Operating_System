...
// Then Make the multiplication process.
void extractAndMultiply(const char *input, int *result) {
...
int main(int argc, char *argv[]) {
    ...
    while(temp){
        ...
        extractAndMultiply(buf, result);

        //Convert result to string and writ it into pipe
        sprintf(sum_str, "%d", result[2]);
        write(pfd1, sum_str, sizeof(sum_str));
        sleep(3);
    }
